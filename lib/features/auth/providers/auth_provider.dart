...
Future<File> copyImageToPermanentDirectory(XFile pickedImage) async {
  final directory = await getApplicationDocumentsDirectory();
  final newPath = '${directory.path}/${pickedImage.name}';
  // Sauvegarde l'image dans le nouveau répertoire
  await pickedImage.saveTo(newPath);
  // Retourne un fichier à partir du chemin du fichier
  return File(newPath);
}
  ...
  if(isRemove) {
    ...
    if (isLogo) {
      ...
      if (picked != null) {
        // Copie l'image dans le répertoire permanent
        File permanentImage = await copyImageToPermanentDirectory(picked);
        _pickedImage = XFile(permanentImage.path);
      }
    } else {
      XFile? xFile = await ImagePicker().pickImage(source: ImageSource.gallery);
      if(xFile != null) {
        // Copie l'image d'identité dans le répertoire permanent
        File permanentImage = await copyImageToPermanentDirectory(xFile);
        _pickedIdentities.add(XFile(permanentImage.path));
      }
    }
    notifyListeners();
  }
